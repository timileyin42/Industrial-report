...
Step-by-step `curl`/`psql` walkthroughs proving each delivery phase's
behavior end-to-end against a local dev stack — device registration,
role/site access control, validation rules, analytics, and hardening.
This is the detailed companion to the top-level [README](../README.md)'s
condensed "Getting Started"; work through it when you need to actually
exercise and verify a specific piece of backend behavior, not just run
the stack.
...
(Phase 3: the versioned `grid_emission_factor` table), `0006_retention_policy.sql`
(Phase 4: a 2-year retention policy on `telemetry` — see `docs/retention.md`,
this is a placeholder pending client confirmation), and every migration
since (hybrid-inverter telemetry fields, cloud-import, sandbox uploads,
tamper-evident audit-log chaining, `ingestion_audit_log` retention — see
the `migrations/` directory for the full, current list).
